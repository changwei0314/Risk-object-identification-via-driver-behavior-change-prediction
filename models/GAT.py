...
class GAT(torch.nn.Module):
    ...
    def __init__(self, num_of_layers=1, num_heads_per_layer=1, num_features_per_layer=1280, add_skip_connection=True, bias=True,
        ...
        layer = GATLayer(
            num_in_features=num_features_per_layer,
            num_out_features=num_out_features,
            num_of_heads=num_heads_per_layer,
...
class GATLayer(torch.nn.Module):
    ...
    # node dimension (axis is maybe a more familiar term nodes_dim is the position of "N" in tensor)
    nodes_dim = 0
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, concat=True, activation=nn.ELU(),
        ...
        # whether we should log the attention weights
        self.log_attention_weights = log_attention_weights
        # for later visualization purposes, I cache the weights here
    ...
    def forward(self, data):
        ...
        nodes_features_proj = self.linear_proj(
            in_nodes_features).view(-1, self.num_of_heads, self.num_out_features)
        ...
        scores_source = (nodes_features_proj *
                         self.scoring_fn_source).sum(dim=-1)
        scores_target = (nodes_features_proj *
                         self.scoring_fn_target).sum(dim=-1)
    ...
    def skip_concat_bias(self, attention_coefficients, in_nodes_features, out_nodes_features):
        ...
        if self.add_skip_connection:  # add skip or residual connection
            ...
            if out_nodes_features.shape[-1] == in_nodes_features.shape[-1]:
                ...
                out_nodes_features += self.skip_proj(
                    in_nodes_features).view(-1, self.num_of_heads, self.num_out_features)
