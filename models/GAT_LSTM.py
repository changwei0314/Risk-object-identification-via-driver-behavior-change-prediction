...
class GAT_LSTM(nn.Module):
    def __init__(self, inputs, time_steps=90):
    ...
    def forward(self, camera_inputs, trackers, device):
        ###########################################
        ...
        camera_inputs = self.backbone.features(camera_inputs.view(-1, c, w, h))
        ...
        for l in range(0, self.time_steps):
            ...
            edge_index = torch.from_numpy(edge_index.astype(np.int64)).to(device)

            # Graph attention network -> output_feature: BxNx1280
            data = (feature_input, edge_index)
            lstm_feature_input = self.gat(data)
